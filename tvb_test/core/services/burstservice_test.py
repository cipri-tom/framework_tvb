...
class BurstServiceTest(BaseTestCase):
    ...
    def test_build_portlet_interface(self):
        ...
        expected_steps = [{'ui_name' : 'TestAdapterDatatypeInput', 'subalgorithm_value' : '', 'subalgorithm_field' : '', 'prefix' : ''}, 
    ...
    def test_update_portlet_config(self):
        ...
        def __update_params(declared_overwrites, expected_result):
            ...
            @param declared_overwrites: a input dictionary in the form {'$$name$$' : '$$value$$'}. Make
                sure $$name$$ has the prefix that is added in case of portlet parameters, namely ADAPTER_PREFIX_ROOT + step_index + actual_name
            ...
            self.assertEqual(expected_result, result, "After update expected %s as 'need_restart' but got %s."%(expected_result, result))
        test_portlet = dao.get_portlet_by_identifier(self.PORTLET_ID)
        portlet_configuration = self.burst_service.new_portlet_configuration(test_portlet.id)
    ...
    def test_update_portlet_config_invalid_data(self):
        """
        Trying an update on a portlet configuration with invalid data should not change the configuration instance in any way.
        """
        test_portlet = dao.get_portlet_by_identifier(self.PORTLET_ID)
        portlet_configuration = self.burst_service.new_portlet_configuration(test_portlet.id)
        ...
        for key in portlet_configuration.__dict__.keys():
            if hasattr(getattr(portlet_configuration, key), '__call__'):
                self.assertEqual(getattr(before_update, key), getattr(portlet_configuration, key), 
                             "The value of attribute %s changed by a update with invalid data when it shouldn't have."%(key,))
    ...
    def test_get_available_bursts_happy(self):
        ...
        returned_test_project_bursts = [burst.id for burst in self.burst_service.get_available_bursts(self.test_project.id)]
        returned_second_project_bursts = [burst.id for burst in self.burst_service.get_available_bursts(second_project.id)]
        self.assertEqual(len(test_project_bursts), len(returned_test_project_bursts), "Incorrect bursts retrieved for project %s."%(self.test_project))
        self.assertEqual(len(second_project_bursts), len(returned_second_project_bursts), "Incorrect bursts retrieved for project %s."%(second_project))
        self.assertEqual(set(second_project_bursts), set(returned_second_project_bursts), "Incorrect bursts retrieved for project %s."%(second_project))
        self.assertEqual(set(test_project_bursts), set(returned_test_project_bursts), "Incorrect bursts retrieved for project %s."%(self.test_project))
    ...
    def test_select_simulator_inputs(self):
        ...
        expected_outputs = [{ABCAdapter.KEY_NAME: simulator_input_tree[0][ABCAdapter.KEY_NAME],
                            ...
                            {ABCAdapter.KEY_NAME: child_parameter,
                             ABCAdapter.KEY_DEFAULT: 'new_value'}]
        self.assertEqual(len(expected_outputs), len(subtree), "Some entries that should not have been displayed still are.")
        for idx, entry in enumerate(expected_outputs):
            ...
            self.assertEqual(expected_outputs[idx][ABCAdapter.KEY_DEFAULT], subtree[idx][ABCAdapter.KEY_DEFAULT], 'Default value not update properly.')
    ...
    def test_sync_burst_launch(self):
        ...
        self.assertEqual(visualizer.fk_algorithm, workflow_step_list[0].fk_algorithm, "Different ids after burst load for visualizer.")
        self.assertEqual(visualizer.static_param, workflow_step_list[0].static_param, "Different static params after burst load for visualizer.")
        self.assertEqual(visualizer.dynamic_param, workflow_step_list[0].dynamic_param, "Different static params after burst load for visualizer.")
    ...
    def test_launch_burst(self):
        ...
        """
        first_step_algo_id, first_step_algo_group = self.flow_service.get_algorithm_by_module_and_class('tvb_test.adapters.testadapter1', 'TestAdapter1')
        ...
        burst_id, _ = self.burst_service.launch_burst(burst_config, 0, first_step_algo_id, self.test_user.id)
        burst_config = dao.get_burst_by_id(burst_id)
    ...
    def test_load_group_burst(self):
        ...
        self.assertTrue(len(datatype_measures) == 3, "Expected 3 Measures. Got %s instead."%(len(datatype_measures), ))
    ...
    def test_launch_burst_invalid_simulator_parameters(self):
        ...
        """
        algo_id, _ = self.flow_service.get_algorithm_by_module_and_class('tvb_test.adapters.testadapter1', 
                                                                         'TestAdapter1')
    ...
    def test_launch_burst_invalid_portlet_analyzer_data(self):
        ...
        Test that burst is marked as error if invalid data is passed to the first step.
        """
        algo_id, _ = self.flow_service.get_algorithm_by_module_and_class('tvb_test.adapters.testadapter1', 'TestAdapter1')
        ...
        self.assertTrue(len(wf_steps) == 2, "Should have exactly 2 wf steps. One for 'simulation' one for portlet analyze operation.")
        simulator_op = dao.get_operation_by_id(wf_steps[0].fk_operation)
        self.assertEqual(simulator_op.status, STATUS_FINISHED, "First operation should be simulator which should have 'finished' status.")
        portlet_analyze_op = dao.get_operation_by_id(wf_steps[1].fk_operation)
        self.assertEqual(portlet_analyze_op.status, STATUS_ERROR, "Second operation should be portlet analyze step which should have 'error' status.")
    ...
    def test_launch_group_burst_no_metric(self):
        ...
        algo_id, _ = self.flow_service.get_algorithm_by_module_and_class(SIMULATOR_MODULE, SIMULATOR_CLASS)
    ...
    def _prepare_and_launch_async_burst(self, length=100, is_range=False, nr_ops=0, wait_to_finish=0):
        ...
        @param is_range: a boolean which switches between a group burst and a non group burst. NOTE: even if
            `is_range` is `True` you still need a non-zero positive `nr_ops` to have an actual group burst
    ...
    def _prepare_and_launch_sync_burst(self):
        ...
        first_step_algo_id, _ = self.flow_service.get_algorithm_by_module_and_class("tvb_test.adapters.testadapter1", 
                                                                                    "TestAdapterDatatypeInput")
        first_step_algorithm = self.flow_service.get_algorithm_by_identifier(first_step_algo_id)
        ...
        self.operation_service.prepare_operations_for_workflowsteps(workflow_step_list, workflows, self.test_user.id, 
                                                            burst_config.id, self.test_project.id, group, operations)
    ...
    def _add_portlets_to_burst(self, burst_config, portlet_dict):
        """
        Adds portlets to a burst config in certain tab position as recieved from a properly 
        syntaxed list of dictionaries.
        @param burst_config: the burst configuration to which the portlet will be added
        @param portlet_dict: a list of dictionaries in the form { 'portlet_id' : [(tab_idx, idx_in_tab), (tab_idx1, idx_in_tab2), ...]
        NOTE: This will overwrite any portlets that are added to the burst in any of the positions recieved in `portlet_dict` 
        """
        for prt_id in portlet_dict:
            positions = portlet_dict[prt_id]
            for pos in positions:
                burst_config.tabs[pos[0]].portlets[pos[1]] = self.burst_service.new_portlet_configuration(prt_id, pos[0], pos[1])
    ...
    def _burst_create_connectivity(self):
        ...
        algo_id, algo_group = self.flow_service.get_algorithm_by_module_and_class(SIMULATOR_MODULE, SIMULATOR_CLASS)
