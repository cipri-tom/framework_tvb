...
class MappedType(model.DataType, mapped.MappedTypeLight):
        ...
        if value is None or isinstance(value, (str, unicode)):
            ...
            if value is not None:
                ...
                if len(instances_arr) > 0:
                    ...
                    msg = ("Could not set '%s' field on '%s' because there is" + 
                           " no '%s' with gid: %s in database.")\
                           %(self.trait.name, inst.__class__.__name__, 
                             self.__class__.__name__, instance_gid)
    ...
    def store_data_chunk(self, data_name, data, grow_dimension=-1, expected_rows=None, 
        ...
        Data will be stored into a data-set with the provided name.
            ...
            ::param expected_rows: Expected number of row elements that will be added to the 
                                   growable dimension. Providing an accurate value for this 
                                   parameter improves storage performance. 
        ...
        store_manager = self._get_file_storage_mng()
        store_manager.append_data(data_name, data, grow_dimension, expected_rows, buffer_shape, close_file, where)
